...
peptide_train <- list.files("data", pattern = ".csv", full.names = T) %>%
  lapply(vroom, show_col_types = FALSE)
peptide_test <- list.files("data", pattern = ".txt", full.names = T) %>%
  lapply(vroom, show_col_types = FALSE)

names(peptide_train) <- list.files("data", pattern = ".csv", full.names = F) %>%
  str_remove_all(".csv") %>%
  str_remove_all("pcp_RT_")

names(peptide_test) <- list.files("data", pattern = ".txt", full.names = F) %>%
  str_remove_all(".txt") %>%
  str_remove_all("masslist_") 
...
for (i in seq_along(train)) {
  ...
  for (j in 1:n_folds) {
    ...
    train_i = train[[i]] %>%
      rename(Mascot_seq=s, RT=rt) %>%
      select(Mascot_seq, RT) %>%
      unique() 
    
    AA = str_split(train_i$Mascot_seq, pattern = "") %>% 
      unlist() %>% 
      unique()
