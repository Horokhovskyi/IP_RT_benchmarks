...
# Ground truth
files_test <- list.files("results/test/achrom/", pattern = ".csv", full.names = T) %>%
  lapply(vroom, show_col_types = FALSE)
names(files_test) <- list.files("results/test/achrom/", pattern = ".csv", full.names = F) %>%
  str_remove(".csv")

# Achrom
pred_achrom <- list.files("results/predict/achrom/", pattern = ".csv", full.names = T) %>%
  lapply(vroom, show_col_types = FALSE)
names(pred_achrom) <- list.files("results/predict/achrom//", pattern = ".csv", full.names = F, recursive = T) %>%
...
# Deep LC
pred_DeepLC <- list.files("results/predict/DeepLC/", pattern = ".csv", full.names = T) %>%
  lapply(vroom, show_col_types = FALSE)
names(pred_DeepLC) <- list.files("results/predict/DeepLC/", pattern = ".csv", full.names = F, recursive = T) %>%
  str_remove_all(pattern = ".csv")
...
# AutoRT
pred_AutoRT <- list.files("results/predict/AutoRT/", pattern = "test.tsv", full.names = T, recursive = T) %>%
  lapply(vroom, show_col_types = FALSE)
names(pred_AutoRT) <- list.files("results/predict/AutoRT/", pattern = "test.tsv", full.names = F, recursive = T) %>%
...
for (i in 1:5) {
  ggsave(gg[[i]], filename = paste0("results/plots/",names(gg)[i],".pdf"), width = 8, height = 6, dpi = "retina")
  ggsave(gg[[i]], filename = paste0("results/plots/",names(gg)[i],".png"), width = 8, height = 6, dpi = "retina")
}

for (i in 6:7) {
  ggsave(gg[[i]], filename = paste0("results/plots/",names(gg)[i],".pdf"), width = 16, height = 9, dpi = "retina")
  ggsave(gg[[i]], filename = paste0("results/plots/",names(gg)[i],".png"), width = 16, height = 9, dpi = "retina")
}
